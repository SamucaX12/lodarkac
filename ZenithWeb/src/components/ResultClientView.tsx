...
export default function ResultClientView({ result, riskScore, detectionsCount, warningsCount, cleanCount, totalItems, conicGradient }: any) {
  ...
  const getMockery = (desc: string) => {
    ...
    if (d.includes("peb hide") || d.includes("módulo oculto")) return "🙈 Escondendo DLL da PEB? Achou que eu não ia varrer a RAM bruta né?";
    if (d.includes("arquivo oculto") || d.includes("deletado")) return "🗑️ Apagou o rastro mas esqueceu que o Windows BAM não perdoa!";
    if (d.includes("bam") && d.includes("executado em")) return "⏰ O horário da execução não mente. Pode chorar pro staff.";
    return null;
  };
    ...
    <div className="max-w-[1600px] mx-auto min-h-screen pb-20 font-sans px-6 animate-in fade-in duration-1000">
      ...
      <header className="flex flex-col md:flex-row items-center justify-between mt-10 mb-10 bg-[#0A0A0A]/80 backdrop-blur-xl p-8 rounded-[3rem] border border-white/5 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col items-center border-x border-white/5 px-20 my-8 md:my-0">
          <div className="flex items-center gap-4">
             <img 
               src={
                 result.game === 'FiveM' ? "https://i.postimg.cc/yYsJwjQ6/image.png" : 
                 result.game === 'ZK' ? "https://i.postimg.cc/h4bqhRnY/image.png" :
                 result.game === 'Quebrada' ? "https://i.postimg.cc/RhvywqmQ/image.png" :
      ...
      <footer className="mt-24 flex flex-col items-center gap-8 opacity-30 grayscale hover:opacity-100 hover:grayscale-0 transition-all duration-1000">
         ...
         <div className="flex items-center gap-12">
            ...
            <img src="https://i.postimg.cc/h4bqhRnY/image.png" alt="ZK" className="h-10 object-contain hover:scale-110 transition-transform" />
            <img src="https://i.postimg.cc/RhvywqmQ/image.png" alt="Quebrada" className="h-10 object-contain hover:scale-110 transition-transform" />
         </div>
