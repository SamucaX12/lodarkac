  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { name: 'Samuca AC', role: 'Founder', img: 'https://i.postimg.cc/3xbh7jhh/image.png' },
          { name: 'Lodark', role: 'Lead Dev', img: 'https://i.postimg.cc/bJ1fQKtQ/image.png' }
        ].map((partner, i) => (
          <div key={i} className="bg-zenith-card p-4 rounded-2xl border border-white/5 flex items-center gap-4 group hover:border-purple-500/30 transition-all">
            <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-purple-500/20 group-hover:border-purple-500/50 transition-all">
              <img src={partner.img} className="w-full h-full object-cover" alt={partner.name} />
            </div>
            <div>
              <p className="text-sm font-black text-white uppercase tracking-tighter">{partner.name}</p>
              <p className="text-[9px] text-purple-400 font-bold uppercase tracking-widest">{partner.role}</p>
            </div>
          </div>
        ))}
      </div>
            ...
            <div className="flex-1 overflow-auto p-6">
              {keys.length === 0 ? (
                ...
                <div className="grid gap-4">
                  {keys.map((key) => (
                    <div key={key._id} className="bg-black/40 border border-white/5 rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 group hover:border-purple-500/30 transition-all">
                      <div className="flex items-center gap-4 flex-1">
                        <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center border border-white/10 overflow-hidden">
                           {key.isSubKey ? (
                             <img src="https://i.postimg.cc/RhvywqmQ/image.png" className="w-full h-full object-cover opacity-60" alt="Avatar" />
                           ) : (
                             <img src="https://i.postimg.cc/3xbh7jhh/image.png" className="w-full h-full object-cover opacity-60" alt="Avatar" />
                           )}
                        </div>
                        <div>
                          <div className="flex items-center gap-3 mb-1">
                            <span className="font-mono text-purple-400 font-bold tracking-widest text-sm">
                              {key.key}
                            </span>
                            <span className="px-2 py-0.5 text-[9px] font-black uppercase rounded-lg bg-white/5 text-gray-500 border border-white/5">
                              {key.plan}
                            </span>
                          </div>
                          <p className="text-[10px] text-gray-600 font-bold uppercase tracking-widest">
                            {new Date(key.createdAt).toLocaleDateString()} • {key.game}
                          </p>
                        </div>
                      </div>
