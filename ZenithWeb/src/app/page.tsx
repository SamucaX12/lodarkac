...
import { Shield, Zap, MessageSquare, Terminal, ChevronRight, Fingerprint, Activity, Cpu, Globe, ShieldCheck, Hash } from 'lucide-react';
  ...
  return (
    <div className="relative w-full min-h-screen bg-[#050505] text-white font-sans selection:bg-violet-500/30 overflow-x-hidden">
      ...
      <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none opacity-20">
        ...
        {/* Matrix Code Effect */}
        <div className="absolute inset-0 overflow-hidden font-mono text-[10px] text-violet-500/20 flex gap-4 justify-around pointer-events-none select-none leading-none">
          {Array.from({ length: 20 }).map((_, i) => (
            <div key={i} className="flex flex-col animate-matrix" style={{ animationDelay: `${i * 0.5}s`, animationDuration: `${10 + Math.random() * 20}s` }}>
              {Array.from({ length: 50 }).map((_, j) => (
                <span key={j} className="mb-4">{Math.random() > 0.5 ? '0' : '1'}{Math.random().toString(16).substring(2, 6)}</span>
              ))}
            </div>
          ))}
        </div>
      ...
      <nav className="relative z-50 flex items-center justify-between px-8 py-8 lg:px-16">
        <div className="flex items-center gap-3 group cursor-pointer">
          ...
          <div className="flex flex-col">
            <span className="text-lg font-black tracking-tighter uppercase italic leading-none">SAMUCA<span className="text-violet-500">.AC</span></span>
            <span className="text-[8px] font-black tracking-[0.4em] text-gray-500 uppercase">Brazilian Forensic Core</span>
          </div>
        </div>
      ...
      <main className="relative z-10 flex flex-col items-center text-center px-6 pt-32 pb-48 max-w-5xl mx-auto">
        ...
        <div className="flex flex-wrap items-center justify-center gap-6 animate-in fade-in slide-in-from-bottom-16 duration-1000 delay-300">
          ...
          <Link 
            href="/verify"
            className="px-12 py-5 bg-black/40 border border-white/10 hover:border-violet-500/50 text-white font-black rounded-2xl flex items-center gap-3 transition-all active:scale-95 group shadow-2xl"
          >
            <Hash size={18} className="text-violet-500 group-hover:rotate-12 transition-transform" />
            Verify PIN
          </Link>
          <a 
          ...
          <div className="relative group">
             ...
             <div className="relative bg-black rounded-[3rem] border border-white/5 p-12 space-y-8 overflow-hidden shadow-2xl">
                ...
                <div className="pt-4 border-t border-white/5 flex items-center justify-between">
                   ...
                   <span className="text-[10px] font-black text-gray-700 uppercase tracking-widest italic">Samuca Forensic v3.0</span>
                </div>
             </div>
          </div>
        </div>
      ...
      <footer className="relative z-10 py-16 px-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 opacity-40 hover:opacity-100 transition-opacity">
         <div className="flex items-center gap-3">
            <Shield size={16} />
            <span className="text-[10px] font-black uppercase tracking-[0.4em] italic">Samuca Advanced Security</span>
         </div>
         <span className="text-[9px] font-black uppercase tracking-widest text-gray-500">© 2026 Samuca AC Core. Total Integrity.</span>
