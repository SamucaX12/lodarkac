...
export function middleware(request: NextRequest) {
  ...
  const protectedRoutes = ['/dashboard', '/pins', '/enterprise', '/logs', '/stats', '/search', '/resellers'];
...
export const config = {
  matcher: ['/dashboard/:path*', '/pins/:path*', '/enterprise/:path*', '/logs/:path*', '/stats/:path*', '/search/:path*', '/login', '/api/((?!auth|config|result|download).*)'],
};
